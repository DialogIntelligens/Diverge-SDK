.PHONY: help sync-version check-version ios-test ios-lint android-build android-test android-paparazzi-record docs-docc sample-ios-open install-hooks ci-local ci-local-ios ci-local-android

help:
	@echo "Diverge SDK make targets:"
	@echo "  make install-hooks     - Enable local pre-commit Git hooks (full CI)"
	@echo "  make ci-local          - Run full local CI (iOS + Android), mirrors GitHub"
	@echo "  make ci-local-ios      - Run full local iOS CI"
	@echo "  make ci-local-android  - Run full local Android CI"
...
android-test:
	cd android && ./gradlew :diverge-sdk:test :diverge-sdk:lint :diverge-sdk:dokkaHtml :diverge-sdk:dokkaJavadoc :sample:assembleRelease :sample:verifyR8PublicApiKeeps :sample:testDebugUnitTest
